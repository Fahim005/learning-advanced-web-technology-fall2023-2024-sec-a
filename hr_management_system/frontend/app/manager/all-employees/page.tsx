...
const EmployeeList = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await axios.get<Employee[]>(`http://localhost:3001/auth/getAllEmployees/${orgID}`,{withCredentials: true,headers: {'Content-Type': 'application/json',}});
  ...
  const handleDelete = async (userId: number) => {
    try {
      ...
      setEmployees(employees.filter(admin => admin.id !== userId));
